...
def visualize_comprehensive_analysis(result, file_name, output_dir, fs=1.0):
	...
	plt.figure(figsize=FIG_SIZE)
	plt.plot(fft_freq_orig, fft_phase_orig, label='Original Signal', color='black', alpha=0.8, linewidth=2.0)
	if fft_freq_slow is not None:
		plt.plot(fft_freq_slow, fft_phase_slow, label='Slow State', color='blue', alpha=0.8, linewidth=2.0)
	if fft_freq_fast is not None:
		plt.plot(fft_freq_fast, fft_phase_fast, label='Fast State', color='red', alpha=0.8, linewidth=2.0)
	plt.xlabel('Frequency [Hz]', fontweight='bold')
	...
	if len(slow_indices) > 0 and len(fast_indices) > 0:
		plt.loglog(f_orig_psd, psd_orig, label='Original Signal', color='black', alpha=0.8, linewidth=2.0)
		plt.loglog(f_slow_psd, psd_slow, label='Slow State', color='blue', alpha=0.8, linewidth=2.0)
		plt.loglog(f_fast_psd, psd_fast, label='Fast State', color='red', alpha=0.8, linewidth=2.0)
	...
	plt.figure(figsize=(10, 6))
	if len(slow_indices) > 0 and len(fast_indices) > 0:
		plt.loglog(f_orig_psd, psd_orig, label='Original Signal', color='black', alpha=0.8, linewidth=2.5)
		plt.loglog(f_slow_psd, psd_slow, label='Slow State', color='blue', alpha=0.8, linewidth=2.5)
		plt.loglog(f_fast_psd, psd_fast, label='Fast State', color='red', alpha=0.8, linewidth=2.5)
		plt.gca().xaxis.set_major_formatter(log_formatter)
		plt.gca().yaxis.set_major_formatter(log_formatter)
	else:
...
def compute_koopman_operator_dmd(X, Y, rank=None):
	...
	try:
		...
		predicted = (modes @ np.diag(eigenvalues) @ modes_pinv @ X.T).T
		predicted = np.real(predicted)
	except Exception as e:
		print(f"预测计算异常: {e}")
		predicted = Y
